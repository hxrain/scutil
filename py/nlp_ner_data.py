...
class types:
    ...
    mining = ('mining', 3)  # 采矿业
    retail = ('retail', 4)  # 批零业
    catering = ('catering', 5)  # 餐饮业
    tourism = ('tourism', 6)  # 旅游业
    amuse = ('amuse', 7)  # 娱乐业
    service = ('service', 8)  # 服务业
    relig = ('relig', 9)  # 宗教
    cultural = ('cultural', 10)  # 文艺
    traffic = ('traffic', 11)  # 交通运输
    communica = ('communica', 12)  # 通信业
    financial = ('financial', 13)  # 金融业
    postal = ('postal', 14)  # 邮政业
    fishery = ('fishery', 15)  # 水产渔业
    industry = ('industry', 16)  # 工业
    gardening = ('gardening', 17)  # 园林绿化
    farming = ('farming', 18)  # 农业种植业
    logistics = ('logistics', 19)  # 物流仓储业
    individual = ('individual', 20)  # 个体户
    org = ('org', 21)  # 团体组织
    env = ('env', 22)  # 环保产业
    medical = ('medical', 23)  # 医疗
    sports = ('sports', 24)  # 体育运动
    construct = ('construct', 25)  # 建筑建造
    tech = ('tech', 26)  # 科技行业
    judicial = ('judicial', 27)  # 司法机关
    breeding = ('breeding', 28)  # 养殖业
    irrigation = ('irrigation', 29)  # 水利
    energy = ('energy', 30)  # 能源
    edu = ('edu', 31)  # 教育行业
    ...
    # NT组份类别优先级
    NX = ('NX', 132)  # 专用于指代未知类型
    NA = ('NA', 133)  # 弱化的特殊名称(地名中含有NO单字尾缀的名称)
    NO = ('NO', 134)  # 组织机构的单字尾缀
    NN = ('NN', 135)  # 企业名称字号
    NZ = ('NZ', 136)  # 专用名词,业务词
    NU = ('NU', 137)  # 数字与序数
    ...
    NS = ('NS', 150)  # 地区类型
    NS1 = ('NS1', 151)  # 省/自治区/直辖市
    NS2 = ('NS2', 152)  # 地级市/州/盟
    NS3 = ('NS3', 153)  # 区/县/旗 (地区/开发区/景区)
    NS4 = ('NS4', 154)  # 乡镇/街道/苏木 (园区/工业园/度假区)
    NS5 = ('NS5', 155)  # 村屯/社区/嘎查/农场/林场/牧场/渔场 (公园/游乐园)

    # # NT层级
    # L0 = 160  # 组织层级0 - 全国性的集团总公司/国家级政府机构(部/司)
    # L1 = 161  # 组织层级1 - 公司主体/组织核心/省市区县级的行政机构(厅/处/局)
    # L2 = 162  # 组织层级2 - 分公司/分厂/村镇级行政机构(科/室/站)
    # L3 = 163  # 组织层级3 - 个体经营摊点/小微组织
    ...
    def _init_():
        ...
        types._maps = {
            ...
            types.NR[1]: types.NR, types.NR1[1]: types.NR, types.NR2[1]: types.NR, types.NR3[1]: types.NR,
            types.NM[1]: types.NM, types.NB[1]: types.NB, types.NL[1]: types.NL,
        }
    ...
    def type(val):
        """对组份类型进行归一化处理"""
        if val in types._itms:
            return types._maps[val[1]]

        if type(val).__name__ in {'set', 'list', 'tuple'}:
            val = max(val, key=lambda t: t[1])

        if val in types._itms:
            return types._maps[val[1]]
        else:
            return types._maps[val]
    ...
    def joint(v, t):
        ...
        def make_set(val):
            rst = set()
            if val in types._itms and val[1] < types.SP[1]:
                rst.add(types.type(val))
            else:
                for v in val:
                    if v not in types._itms or v[1] < types.SP[1]:
                        continue
...
nt_tails = {
    # 工矿企业
    '分店': {'.': {types.NB}, '-': {},
           '+': {'分号', '分厂', '分矿', '分校', '分公司', '子公司', '分司', '分工司', '支公司', '分部', '分局', '分院', '分所', '直属分局', '支局', '分园', '子厂', '分场', '分社',
                 '分站', "分会", "子站", "分队", "分处", '分区', '分台', '(分)局', '名胜区分公司', '机场分公司', '林场分公司', '牧场分公司', '火车站分公司', '火车站分社',
                 '中心分社', '中心支局', '中心支公司', '支校', '分校园', '分(院)', '分市场', '小分队'}},
    '工厂': {'.': {types.NM, types.industry}, '-': {},
           '+': {'加工厂', '化工厂', '工区', '件厂', '剂厂', '布厂', '床厂', '总厂', '机厂', '材厂', '板厂', '水厂', '油厂', '泵厂', '净水厂', '纯水厂', '纯净水厂',
                 '炉厂', '煤厂', '瓦厂', '瓷厂', '石厂', '砖厂', '管厂', '箱厂', '粉厂', '纸厂', '织厂', '肥厂', '胶厂', '船厂', '茶厂', '药厂', '表厂',
                 ...
                 "香厂", "实验厂", '选煤厂', '车间', '采油厂', '盐厂', '窗纱厂', '火柴厂', '制作厂', '工加厂', '分车间', '百货厂'}},
    '电厂': {'.': {types.NM, types.energy}, '-': {},
           '+': {'发电厂', '电站', '发电站', '水电站', '核电站', '供电局', '供电所', "风电场", '中心供电所'}},
    '公司': {'.': {types.NM, types.com}, '-': {},
           '+': {'总公司', '有限公司', '有限责任公司', '责任公司', '股份公司', '集团公司', '集团有限公司', '集团有限责任公司', '集团总公司', '株式会社', '顾问有限公司', '连锁有限公司',
                 '服务公司', '工程公司', '实业有限公司', '股份有限公司', '保险股份有限公司', '开发有限公司', '设备有限公司', '发展有限公司', '经纪有限公司', '管理有限公司', '工程有限公司',
                 '建设发展有限公司', '设计有限公司', '建设有限公司', '制造有限公司', 'LTD', '劳司', '有限责任工司', '农工商联合公司', '农工商公司', '农工商总公司', '酒店管理有限公司',
                 '旅行社有限公司', '设计院有限公司', '研究院有限公司', '医院有限公司', '服务区有限公司'}},
    '中心': {'.': {types.NM, types.gov, types.service}, '-': {},
           '+': {'服务中心', '管理中心', '培训中心', '运动中心', '旅游开发管理中心', '行政事务管理中心', '安置中心', '副中心', '超市中心'}},
    '煤矿': {'.': {types.NM, types.mining}, '-': {'采矿', '探矿', '洗矿', '矿长', '矿产'},
           '+': {'金矿', '铁矿', '气矿', '盐矿', '石矿', '砂矿', '硅矿', '钙矿', '钛矿', '钨矿', '铅矿', '钼矿', '铀矿', '铜矿', '铝矿', '锡矿', '硫铁矿',
                 '锌矿', '锰矿', '联营矿', '金属矿', '原料矿', '有色矿', '露天矿', '磷矿'}, },
    ...
    '办公室': {'.': {types.NM, types.gov}, '-': {},
            '+': {'管理办', '项目办', '研究室', '教学研究室', '政策研究室', '工作办', '法制办', '开发办', '综合开发办', '政府办', '接待办', '建设管理办', '领导小组办', '建设办', '政府办公室'}},
    # 学校
    '幼儿园': {'.': {types.NM, types.edu}, '-': {},
            '+': {'幼稚园', '机关幼儿园', '附属幼儿园', '中心幼儿园', '午托所', '午托班', '幼儿班', '托管班', '托育班', '看护班', }},
    '培训班': {'.': {types.NM, types.edu}, '-': {},
            '+': {'委托班', '教学班', '补习班', '训练班', '辅导班', "讲师团", }},
    '学校': {'.': {types.NM, types.edu}, '-': {'学业'},
           '+': {'小学', '小学校', '中学', '中学校', '高中', '中心校', '总校', '中心学校', '武校', '盲校', '网校', '聋校', '联校', '公学', '职业中学', '附属中学', '中心小学校', '中心中学',
                 '民族中学', '初级中学', '高级中学', '职业高级中学', '实验中学', '中小学', '完全小学', '附属小学', '中心小学', '民族小学', '实验小学', '初级中学校', '中心校', '中心完全小学',
    ...
    '职校': {'.': {types.NM, types.edu}, '-': {},
           '+': {'艺校', '驾校', '进修校', '体校', '技校', '鼓校', '业校', '中专', '职业中专', '卫校', '农校', '农广校', '农机校', '联合校', '技工学校', '技术学校',
                 ...
                 '医专', '行政管理学校', }},
    '大学': {'.': {types.NM, types.edu}, '-': {'大学生', '大学城', '大学路', '大学东路', '大学西路', '大学士', '大学南路', '大学北路', '大学中专', '大学街', '大学堂', '大学仕', '大学塘'},
    ...
    '委员会': {'.': {types.NM, types.gov}, '-': {},
            '+': {'工会委员会', '工作委员会', '管理委员会', '常委会', '人大常委会', '管委会', '组委会', '委会', '区委', '县委', '纪委', '团委', '市委', '省委', '地委',
                  '工作委', '常务委', '政法委', '管理委', '党委', '卫健委', '发改委', '州委', '工会委', '工委', '建委', '教委', '文化委', '文卫委', '旅发委', '大学委员会',
                  '纪工委', '纪检委', '计生委', '生态管理委员会', '建设管理委员会', '筹委会', '监事会', '中心委员会', '中心委', '中心校委员会', '中心局委员会',
                  }},
    '居委会': {'.': {types.NM, types.gov}, '-': {},
            '+': {'居民委', '居民委员会', '居民居委会', '民居委员会', '居民委会', '治理委员会', '居民事务中心', '居委', '街民委员会', '居民管理委员会', '家委会', '街道委员会', '街道办事处',
                  '办事处', '街道办', '街道', '中心街道', '类似居委会', '商务公寓工作站', '民主议事会', '居民议事会', '社区工作站', "团场"}},
    '村委会': {'.': {types.NM, types.gov}, '-': {},
            '+': {'联合村委会', '村民委员会', '村委', '村民委', '村民小组', '村名委员会', '村委员会', '村民村委员会', '村民委会', '直辖村民小组', '渔委会', '牧委会', '嘎查委会', '嘎查人民委员会',
                  '嘎查委员会', '牧民委员会', '村民议事会'}},
    '十字会': {'.': {types.NM, types.org, types.medical}, '-': {},
            '+': {'红十字会', '红十会'}},
    '工会': {'.': {types.NM, types.gov}, '-': {},
           '+': {'机关工会', '分工会', '总工会', '教育工会', "工会小组", "联合工会"}},
    '董事会': {'.': {types.NM, types.org}, '-': {},
            '+': {'校董会'}},
    # 金融机构
    ...
    '合作社': {'.': {types.NM, types.industry, types.farming}, '-': {},
            '+': {'种植专业合作社', '养殖专业合作社', '合社', '综合社', '联合社', '经济联合社', '合作社联合社', '专业合作社联合社', '供销合作社联合社', '合作社联社',
                  '联社', '合作联社', '专业合作联社', '信用合作联社', '农村信用合作联社', '经济社', '合作经济社', '专业合作社', '股份合作社', '互助合作社', '经济合作社',
                  '信用合作社', '供销合作社', "股份经济合作社", '经营所', '中心供销合作社'}},
    '营业部': {'.': {types.NM, types.retail}, '-': {},
            '+': {'供销社', '经销社', '经营部', '经部', '销部', '营部', '配部', '经销部', '营销部', '购销部', '销售部', '零售部', '租赁站', '购销站', '火车站营业部', '门市部', '火车站门市部',
    ...
    '医院': {'.': {types.NM, types.medical}, '-': {},
           '+': {'中医院', '妇产医院', '中医医院', '康复医院', '附属医院', '职工医院', '中心医院', '总医院', '人民医院', '动物医院', '宠物医院', '精神病医院', "血站",
                 ...
                 '中心卫生院', '中心血站', '脑科医院', '眼科医院', '连锁门诊部', '中医堂', '医院东院', '医院南院', '医院西院', '医院北院'}},
    '药店': {'.': {types.NM, types.medical, types.retail}, '-': {},
           '+': {'国药号', '药号', '药房', '大药房', '药铺', '药行', '药材行', '药材站', '药材栈', '药庄', '药妆店', '药堂', '药品部', '药品超市', '药品站',
                 ...
                 '中药材部', '中草药馆', '中草药行', '中药铺', '药品行', '藥號', '中心药房', '中心大药房', '中心药店', '连锁药店', '连锁药房'}},
    # 文化娱乐
    ...
    '饭店': {'.': {types.NM, types.catering}, '-': {},
           '+': {'快餐', '餐饮店', '食店', '冷饮店', '饼店', '饭馆', '酒楼', '大酒楼', '大饭店', '酒家', '大酒家', '食堂', '大食堂', '职工食堂', '餐厅', '烧烤广场', '啤酒广场', '美食广场',
                 '餐饮广场', '西餐厅'}},
    '茶馆': {'.': {types.NM, types.catering}, '-': {},
           '+': {'茶社', '茶楼', '茶舍', '茶座', '茶都', '茶莊', '茶艺居', '茶艺店', '茶艺苑', '茶艺馆', '茶饮店', '茶饮坊', '茶叶店', '茶居', '茶荘', '茶庄', '茶庄行', '茗茶轩', '茶轩', '茶饮室', '茶艺坊', '茶艺室',
                 '咖啡厅', '咖啡馆', '茶坊店', '饮水店', '茶舫', "茶室", "茶坊", }},
    '农家宴': {'.': {types.NM, types.catering}, '-': {'宴席', '宴会', '宴请'},
            '+': {'全猪宴', '渔家宴', '藏家宴', '饺子宴', '全羊宴', '博食宴', '豆腐宴', '鸽宴'}},
    ...
    '食品店': {'.': {types.NM, types.retail, types.catering}, '-': {},
            '+': {'新鲜屋', '甜品店', '饮品店', '汉堡店', '包子店', '水果店', '蛋糕房', '食品门市', '食品超市', '水果超市', '副食部', '餐饮部', '蛋糕店', '茶店', "包子铺", "馕店", "鱼店",
                  ...
                  "甜品站", "果行", '海鲜港', '南饼屋', '南食店', '食品城', '饺子城', '中心粮店', '美食花园', '啤酒花园'}},
    ...
    '宾馆': {'.': {types.NM, types.tourism, types.catering}, '-': {},
           '+': {'商务宾馆', '旅馆', '会馆', '公馆', '酒店', '大酒店', '旅店', '招待所', '客栈', '度假村', '度假邨', '旅社', '客舍', '宾舍', '宿舍', '旅舍', '青年旅舍', '火车站旅馆',
                 '火车站宾馆', '中心酒店', '连锁酒店'}},
    '民宿': {'.': {types.NM, types.tourism}, '-': {},
           '+': {'公寓', '庄园', '农庄', '居士林', '蒙古大营', '傣家园', '农家园', '残疾人之家', '老年公寓', '商务公寓',
    ...
    '寺院': {'.': {types.NM, types.relig}, '-': {},
           '+': {'洞召', '清真寺', '佛堂', '佛寺', '佛禅寺', '古寺', '圆通寺', '地藏寺', '大寺', '女寺', '宁禅寺', '少林寺', '广福寺', '普济寺', '清凉寺', '清真女寺',
                 '观音寺', '龙泉寺', '中寺', '禅寺'}},
    '教会': {'.': {types.NM, types.relig}, '-': {},
           '+': {'天主堂', '教堂', '天主教下会点', '天主教公所', '天主教堂', '天主教活动场所', '天主教祈祷所', '天主教爱国会', '基督教三自爱国会', '基督教青年会', '教区', }},
    # 洗浴会所
    ...
    '俱乐部': {'.': {types.NM, types.service}, '-': {},
            '+': {'足球俱乐部', '体育俱乐部', '青少年体育俱乐部', '健身俱乐部', '棋牌室', '舞厅', '歌舞厅', '歌厅', '之家', '地带', '骑行者之家', '休闲汇',
                  '娱乐汇', '音乐汇', '渔排垂钓', '娱乐广场', '休闲广场', }},
    '理发店': {'.': {types.NM, types.service}, '-': {},
            '+': {'发典', '发厅', '发廊', '美发店', '美甲店', '美容店', '按摩店', '足浴店', '美容馆', '养生馆', "发室", "理发室", "发屋", "发艺店",
                  "养生店", "足疗店", "造型店", '发舍', '香舍', "美体店", "保健馆", "服务馆", "健身馆", "按摩馆", "理疗馆", '美发城'}},

    # 商业销售
    '商店': {'.': {types.NM, types.retail}, '-': {'店内', '门市内', '市面'},
    ...
    # 地标店铺
    '万达店': {'.': {types.NM, types.retail}, '-': {},
            '+': {'火车站超市', '超市店', '中心店', '中心文具店', '中学店', "小学店", "小区店", "国际店", '名店', '站店', '堂店', '院店', '苑店', '园店', '新店', '支店', '阁店',
                  '轩店', '斋店', '商贸区店', '少年宫园', '少年宫店', '市政府园', '豪宅店', '华润店', '华能店', '中铁店', '中建店', '师专店', '师大店'}},

    '书店': {'.': {types.NM, types.retail}, '-': {},
           '+': {'图书店', '新华书店', '小学书店', '中学书店', '高中书店', '大学书店', '技校书店', '机场书店', '广场书店', '学院校园书店', '职院校园书店', '卫院校园书店', '学校校园书店',
                 '校园书店', '中心校校园书店', '小学校园书店', '中学校园书店', '校区校园书店', '高中校园书店', '初中校园书店', '中校园书店', '中新华书店', '中书店', '连锁书店'}},
    '商贸行': {'.': {types.NM, types.retail}, '-': {},
            '+': {'寄卖行', '拍卖行', '电器行', '典当行', '贸易行', '电脑行', '通讯行', '租赁行', '装饰行', '批发行', '量贩', '加油港', '新干线汽车港', '鲜花港', "道馆", "家居馆",
                  '加油城', '总代理', }},
    '商贸城': {'.': {types.NM, types.retail}, '-': {},
    ...
    '商行': {'.': {types.NM, types.retail}, '-': {},
           '+': {'琴行', '茶行', '车行', '酒行', '金行', '总汇', '名酒汇', '美食汇', '大全', '大全店', '总汇店', "烟店", "玻璃店", "陶瓷店", "数码店", "家私店", '劳保店', '办公店',
                 "烟花爆竹店", "纺店", "家纺店", "裁缝店", "商贸店", "汽配店", "轮胎店", "时装店", "男装店", "设计店", "水产店", "文体店", "制作店", "通信店", "汽修店", '芳叔店', '胡同店', '五交化店',
    ...
    '仓库': {'.': {types.NM, types.logistics}, '-': {'库内', '库存', '库中'},
           '+': {'储备库', '粮食储备库', '直属库', '粮库', '仓储部', '仓储场', '仓储站', '粮站', '货仓', '资料仓', '配送仓', '供应仓', '保税仓', '保管仓', '前置仓', '副食仓', '化肥仓', '工艺仓', '布仓',
                 '批发仓', '批发货仓', '物资仓', '盐仓', '监管仓', '粮仓', '肥料仓', '中心库', '(配送)库', '中转库', '仓储库', '供应总库', '保税库', '保鲜库', '储备库', '储藏库', '米粮库', '中心粮库',
    ...
    '工作室': {'.': {types.NM, types.tech}, '-': {},
            '+': {'制作工作室', '设计工作室', '实验室', '设计室', '婚典', '中心试验室', "试验室", '中心实验室', '中心化验室'}},
    ...
    '服务队': {'.': {types.NM, types.service}, '-': {},
            '+': {'维修队', '劳务队', '装卸队', '施工队', '工程队', '土石方工程队', '建筑工程队', '建筑队', '测绘队', '修缮队', '安装队', '设计队', '养护队', '城调队', '农业队',
                  '救护队', "专业队", "工作队", "绿化队", "监察队", "搬运队", "测量队", '生产队', '牧业队', '林业队', '中队', '井队', '大队', '总队', '支队', '车队', '火车站装卸队'}},
    ...
    # 农牧业
    "鱼场": {'.': {types.NM, types.fishery}, '-': {},
           '+': {'渔塘', '鱼池', '渔港', '养殖基池', '养鱼池', '养鱼塘', "渔场", '鱼塘', '鱼苗场', '鱼原种场', '水产渔场', '鱼苗鱼种场', '鱼种场', '水产试验场'}},
    '种植场': {'.': {types.NM, types.farming}, '-': {},
            '+': {'花圃', '苗圃', '茶园', '果园', '种植园', '农场', '木场', '林场', '果场', '苗场', '茶场', '菜场', '花木场', '竹木场', '储林场', '农作物种植场', "实验农场", '中心苗圃',
                  ...
                  '桔场', '茶叶示范场', '园艺试验场', '林场区', '林木场', "草籽场", '农业场', '花卉繁殖场', '果树繁殖场', '良桑繁殖场', '原种繁殖场', '种籽繁殖场', '苗种繁殖场', '园艺繁殖场',
                  '农特场', '林特场', '农业示范场', '种养示范场', '开发示范场', '栽培示范场', '生态示范场', '科技示范场', '果树示范场', '柑桔示范场', '良种示范场', '林特示范场', '园艺示范场', '试验示范场',
                  '农业试验场', '综合试验场', '果树试验场', '良种试验场', '种苗试验场', '育苗试验场', '林草试验场', '种植区', '林苗场', '芦苇场', '采育场', '储木场', '围垦场', '养植场', '培植场',
                  '作物场', '示范场', '种植专场', '林场社区', '林业总场社区', '农场社区'}},
    '养殖场': {'.': {types.NM, types.breeding}, '-': {},
            '+': {'猪场', '羊场', '马场', '鸡场', '养殖基地', '水产养殖场', '猪养殖场', '生猪养殖场', '垦殖场', '牧场', '原种场', '养殖埸', '养殖总埸', '养殖总场', '农牧场', '种羊场', '水产场', '山羊场',
                  '养殖小区', '养猪小区', '牧业小区', '养殖舍', '犬舍', '猫舍', '鸡舍', '鸽舍', "种养场", "饲养场", "繁育场", "畜牧发展场", "肉牛发展场", "养殖发展场", "种猪场", "进化种猪场", '奶山羊场',
                  '黄牛良种繁育场', '良种繁育场', '放马场', '畜牧场', '绵羊育种场', '种畜场', '山羊选育场', '选育场', '畜禽良种场', '畜牧农场', '林牧场', '森林牧场', '畜牧良种场', '良繁场', '驯养繁殖场',
                  ...
                  '养蚕场', '养殖试验场', '增养殖试验场', '种养殖试验场', '畜牧试验场', '鸽场', '养龟场', '种鳖场', '鳗场', '畜禽试验场', '种猪试验场', '鱼种试验场', '种马场', '种蜂场', '种蓄场', '畜场'}},
    ...
    '水库': {'.': {types.NM, types.irrigation}, '-': {},
           '+': {'水站', '翻水站', '扬水站', '供水站', '水利站', "水务站", '抽水站', '电排站', '供水场', '淡水场', '加水场', '主干渠', '分干渠', '引水渠', '输水渠', '总干渠', '支干渠', '干渠', '支渠'}},
    ...
    '中共': {'.': {types.NF, types.gov}, '-': {},
           '+': {'中国共产党', '共产党', '共青团'}},
    '国营': {'.': {types.NF, types.com}, '-': {},
           '+': {'中交', '中建', '国网', '国电', '中烟', '中电', '中电建', '中铁建', '国家', '华能', '华电', '中铁', '兵团', '中粮', '生产建设兵团'}},

    # 地区/方位
    '中央': {'.': {types.NS}, '-': {},
           '+': {'华北', '东北', '华东', '华中', '华南', '西南', '西北', '东部', '中部', '西部', '中南', '北方', '南方', '东南', '中区', '东区', '南区', '西区', '北区',
                 '苏木', '嘎查', '以东', '以西', '以南', '以北', '嘎查村', '苏木村', '中心村', '苏木乡', '自治县', '自治州','自治旗', '自治区', '民族乡', '新村', '古村', '民族苏木'}},
    '大街': {'.': {types.NA}, '-': {},
           '+': {'东', '南', '西', '北', '省', '市', '区', '县', '州', '盟', '旗', '乡', '村', '镇', '路', '街', '巷', '道', '大道', '线', '段'}},

    # 开发区/地区/园区
    '地区': {'.': {types.NS3}, '-': {},
           '+': {'新建区', '开发区', '新区', '托管区', '保护区', '经开区', '管区', '保护开发区', '保税区', '边境经济合作区', '产业集聚区', '经发区',
                 ...
                 '合作经济开发区', '合作区', '互市贸易区', '华侨经济开发区', '华侨经济区', '华侨投资区', '化工集中区', '火炬高技术开发区', '技术经济开发区', '建设区', '经济合作区',
                 ...
                 '民营区', '农业高新区', '农业经济开发区', '农业开发区', '农业综合开发区', '汽车产业开发区', '商贸经济示范区', '商贸示范区', '商务区', '新兴技术开发区', '合作产业开发区',
                 ...
                 '商贸物流枢纽区', '投资区', '经技区'}},
    '景区': {'.': {types.NS3, types.cultural}, '-': {},
           '+': {'风景名胜区', '风景区', '风景游览区', '名胜风景区', '旅游度假区', '旅游渡假区', '旅游经济开发区', '旅游景区', '旅游开发区', '温泉风景区', '温泉疗养旅游开发区',
                 ...
                 '商业文化旅游中心区', '生态文化旅游示范区', '文化景区', '生态区'}},
    '片区': {'.': {types.NS4, types.medical}, '-': {},
           '+': {'疗区', '监区', '矿区', '新片区', '病区', '居民区', '小区', '住宅区', '分蓄洪区', '蓄洪区', '洪泛区', }},
    '社区': {'.': {types.NS5, types.gov}, '-': {},
           '+': {'集体资产社区', '组团虚拟社区', '直属社区', '直属虚拟社区', '虚拟社区', '虚拟生活区', '生活区', '类似社区', '联合小区', '产业社区', '工业社区', '类似乡级单位', '特殊社区',
                 '直辖村级区划', '新型农村社区', '中心社区', '虚拟劳管所', '类似村', '公司村', '社区村', '农场村', '实验村', '渔场村', '林场村', '区块社区', '城市社区', '文化社区',
                 '火车站社区', '花园', }},
    '管理区': {'.': {types.NS3, types.NM, types.cultural}, '-': {},
            '+': {'特别管理区', '风景名胜管理区', '旅游管理区', '生态管理区', '特别行政管理区', '农垦管理区', '华侨管理区', '经济管理区', '垦区', '围垦区', '资源区', '管护区', '林区',
                  '航空港区', '保税港区', '核心港区', '灌区', '分灌区', '销售片区', '旅游区', '文化旅游区', '海湾旅游区', '生态文化旅游区', '风景旅游区', '生态旅游区', '边贸旅游区', }},
    '园区': {'.': {types.NS4, types.NM, types.service, types.industry}, '-': {},
           '+': {'采区', '城市工业园区', '发展园区', '高教园区', '高科技园区', '高科园区', '高新技术产业园区', '工业园区', '工业园小区', '农高区', '科创区',
                 '国家农业园区', '化工园区', '教育园区', '经济工业园区', '酒业园区', '科工贸园区', '科技园区', '空港物流园区', '临港化工园区', '临港物流园区', '牧业园区', '农业高新技术园区', '警备区',
                 '农业科技园区', '农业园区', '汽车园区', '商务产业园区', '深加工园区', '生态工业园区', '示范小区', '微电子小区', '温泉休闲商务产业园区', '物流园区', '物业区', '现代农业园区', '空港区', '功能区',
                 '院区', '中心商务区', '科技商务区', '专利技术园区', '工业园管理区', '高新技术示范园区', '承接产业转移集中示范园区', '集中示范园区', '现代农业示范园区', '农业示范园区', '出口创汇示范园区', '产业园区',
                 '新产业园区', '新技术产业园区', '新型工业园区', '循环经济产业园区', '制造工业园区', '现代产业园区', '生态农业园区', '现代服务业园区', '农副产品加工交易园区', '交易园区', '农业园区',
                 '商务商贸街区', '现代农牧业园区', '农牧业园区', '生物科技产业园区', '农机产业园区', '装备制造产业园区', '现代农业产业园区', '农业产业园区', '家纺产业园区', '能源化工园区', '厂区',
                 '国家农业科技园区', '港区', '轻工业园区', '新材料园区', '高新产业园区', '职业教育园区', '循环经济园区', '循环园区', '安置区', '临空综合经济园区', '新兴产业工业园区', '工业示范区', '生态经济园区', '储粮区',
                 '港务区', '国际港务区', '实验区', '作业区', '加工区', '国际物流加工区', '工业加工区', '出口加工区', '外向型工业加工区', }},
    ...
    '厂': {'.': {types.NO, types.industry}, '-': {'厂房', '厂长', '厂内', '厂外', '间距', '间隔', '我厂'}, '+': {}},
    '店': {'.': {types.NO, types.retail}, '-': {}, '+': {}},
    '部': {'.': {types.NO}, '-': {'部门', '部长', '部分', '我部'}, '+': {}},
    '馆': {'.': {types.NO, types.service}, '-': {'馆长', }, '+': {}},
    '所': {'.': {types.NO}, '-': {'所长', '所以', '所得', '本所', '我所'}, '+': {'片区所'}},
    '局': {'.': {types.NO, types.gov}, '-': {'局长', '局部', '局座', '局域', '本局', '我局', '及局', '与局', '和局'}, '+': {}},
    '矿': {'.': {types.NO}, '-': {}, '+': {}},
    '城': {'.': {types.NO}, '-': {'城墙', '城门'}, '+': {}},
    '站': {'.': {types.NO, types.service}, '-': {}, '+': {}},
    '院': {'.': {types.NO}, '-': {'院长', '院方', '院子', '院内'}, '+': {}},
    ...
    '(普通合伙)': {'.': {types.NL, types.financial}, '-': {},
               '+': {'(特殊合伙)', '(有限合伙)', '(特殊普通合伙)', '(有限合伙企业)', '(普通合伙企业)', '(特殊普通合伙企业)'}},
    '(有限公司)': {'.': {types.NL, types.com}, '-': {},
               '+': {'(有限责任)', '(有限责任公司)', '(股份有限公司)', '(无字号名称)', '(公司)'}},
    ...
    # 不规范的尾缀特征,暂不使用
    # '茶艺': {'.': {types.NM, types.catering}, '-': {},
    #        '+': { '茶饮','茶港', '茗茶庄','渔村','江渔村',}},

    # 小微餐饮排档,不使用
    # '档口': {'.': {types.NM, types.catering, types.retail}, '-': {},
    #        '+': { '干调档口', '面档口', '饭档口', '家常菜','私房菜','农家菜馆','大排档','排档'}},

    # 小微服务业,暂不使用
    # '酒吧': {'.': {types.NM, types.service}, '-': {},
