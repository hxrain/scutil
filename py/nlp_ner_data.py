...
nt_tails = {
    ...
    '工坊': {'.': types.tags_NO, '-': {},
           '+': {'油坊', '磨坊', '酒坊', "作坊", "烘焙坊", '工场', '联盟'}},
    '加工场': {'.': types.tags_NM, '-': {},
            '+': {'总场', '梁场', '沙场', '煤场', '盐场', '石场', '砂场', '屠宰场', '制梁场', '采石场', '停车场', '停车埸', '洗车场', '加工埸', "料场", "预制场", '拆解场', '存车场',
                  '拆车场', '试验场', '洗车坊'}},
    '基地': {'.': types.tags_NM, '-': {},
    ...
    '学校': {'.': types.tags_NM, '-': {'学业'},
           '+': {'小学', '小学校', '中学', '高中', '职业高中', '中心校', '总校', '中心学校', '武校', '盲校', '网校', '聋校', '联校', '公学', '职业中学', '附属中学', '中心小学校', '中心中学',
                 ...
                 '高级中学校', '进修学校', '实验小学校', '实验学校', '学园', '师范附小', '初中', '实验初中', '附属初中', '中心初中', '中心总校', '中心完小', '中心初级中学',
                 '二校区', '中心校区', '学区', '基地校', '初中校', '高中校', '小学东校', '小学西校', '小学南校', '小学北校',
                 '中学东校', '中学西校', '中学南校', '中学北校', '学校东校', '学校西校', '学校南校', '学校北校'}},
    '校区': {'.': types.tags_NO, '-': {},
           '+': {'东校区', '南校区', '西校区', '北校区', '新校区', '小学部', '初中部', '高中部', '附小', '完小', '联小', }},
    '职校': {'.': types.tags_NM, '-': {},
    ...
    '省委': {'.': types.tags_NO, '-': {},
           '+': {'市委', '区委', '县委', '旗委', '州委', '村委', '纪委', '团委', '地委', '委会', '盟委', '村委会', '居委', '校友会', '商会', '学会', '街道办事处'}},
    '居委会': {'.': types.tags_NM, '-': {},
    ...
    # 金融机构
    '信用社': {'.': types.tags_NO, '-': {},
            '+': {'信用联社', '农村信用联社', '中心信用社', '储蓄所', '火车站储蓄所', '分理处', '火车站分理处', }},
    '银行': {'.': types.tags_NM, '-': {'行走', '行为'},
           '+': {'分行', '总行', '商业银行', '农村商业银行', '农村合作银行', '工商银行', '邮政储蓄银行', '村镇银行',
                 '基金会', '发展基金会', '教育基金会'}},
    '支行': {'.': types.tags_NB, '-': {},
    ...
    '营业部': {'.': types.tags_NM, '-': {},
            '+': {'供销社', '经销社', '经营部', '经销部', '营销部', '购销部', '销售部', '零售部', '租赁站', '购销站', '火车站营业部', '门市部', '火车站门市部',
                  '销售处', '售票处', '经销处', '总经销', '中心供销社', '中心售票处'}},
    '销部': {'.': types.tags_NO, '-': {},
           '+': {'经部', '配部', '总汇', }},
    ...
    '医院': {'.': types.tags_NM, '-': {},
           '+': {'中医院', '妇产医院', '中医医院', '康复医院', '附属医院', '职工医院', '中心医院', '总医院', '人民医院', '动物医院', '宠物医院', '精神病医院', "血站",
                 '专科医院', '眼科医院', '骨科医院', '口腔医院', '蒙医院', '藏医院', '美容院', '卫生院', '疗养院', '保健院', '妇幼保健院', "医务室",
                 '中医诊所', '口腔门诊', '医院新院', '中心血库', '血库', "防治院",
                 '中心卫生院', '中心血站', '脑科医院', '眼科医院', '连锁门诊部', '医院东院', '医院南院', '医院西院', '医院北院'}},
    '医药商店': {'.': types.tags_NM, '-': {},
             '+': {'药品超市', '医药量贩', '医药市场', }},
    '门诊部': {'.': types.tags_NO, '-': {},
            '+': {'药店', '口腔门诊部', '卫生室', '药号', '药房', '大药房', '药铺', '药行', '药材行', '药材站', '药材栈', '药庄', '药妆店', '药品部', '药品店', '药具站',
                  '草药部', '草药站', '膏药老店', '老药铺', '老膏药铺', '膏药铺', '新特药部', '药部', '大药店', '大药行', '国药馆', '医药馆', '医药部', '医药行',
                  '医药站', '医药店', '中药馆', '中西药行', '中药饮片部', '中药部', '中药配方部', '中药行', '国药号', '中药材部', '中草药馆', '中草药行', '中药铺',
                  '药品行', '藥號', '中心药房', '中心大药房', '中心药店', '连锁药店', '连锁药房', '连锁大药房', '药品站', '中医堂', '中医所', '卫生所', '门诊', '诊所', }},
    # 文化娱乐
    '影剧院': {'.': types.tags_NM, '-': {},
    ...
    '咖啡馆': {'.': types.tags_NM, '-': {},
            '+': {'茶艺居', '茶艺店', '茶艺苑', '茶艺馆', '茶饮店', '茶饮坊', '茶叶店', '茶庄行', '茗茶轩', '茶饮室', '茶艺坊', '茶艺室',
                  '咖啡厅', '茶坊店', '饮水店', }},
    '茶馆': {'.': types.tags_NO, '-': {},
           '+': {'茶楼', '茶座', '茶都', '茶莊', '茶居', '茶荘', '茶庄', '茶轩', '茶舫', "茶室", '饮舍', "饮吧", "奶店", '茶舍', '网吧', '网咖', "味馆", '茶社', '茶店', }},
    '农家宴': {'.': types.tags_NM, '-': {'宴席', '宴会', '宴请'},
            '+': {'全猪宴', '渔家宴', '藏家宴', '饺子宴', '全羊宴', '食宴', '豆腐宴', '鸽宴'}},
    '饭庄': {'.': types.tags_NM, '-': {},
           '+': {'面莊', '饭莊', '鱼莊', '羊肉府', '美食府', '食府', '美食荟', '美食都', '干锅居', '火锅居', '砂锅居', '美食居', '饺子居', "饭堂",
                 '美食林', '麻辣烫店', '粥荘', '食荘', '饭荘', '鱼荘', '面荘', '食庄', '食莊', '豆花庄', '面庄', '面馆', '鲜味渔庄', '渔家庄', '渔庄', '烤羊庄', '羊庄',
                 '炖鱼村', '江鱼村', '火锅村', '烙馍村', '美食村', '莜面村', '饺子村', "餐部", "餐饮馆", '肥牛城'}},
    '酒馆': {'.': types.tags_NO, '-': {},
           '+': {'汤馆', '粉馆', '肉馆', '鱼馆', '饼家', '鱼庄', '馍房', "奶吧", "水吧", "烤吧", "饼屋", "串店", "汤店", '粮店', '油店', '粉店', '肉店', '菜店', '面店', '鸡店', '鸭店', "果行",
                 "馍店", "米店", "食坊", '庄园', '快餐', '饼店', '酒家', '农庄', '鱼府', "馕店", "鱼店", '副食部', '餐饮部', '蛋糕店', "包子铺", }},
    ...
    '商店': {'.': types.tags_NM, '-': {'店内', '门市内', '市面'},
           '+': {'门店', '专门店', '专营店', '专业店', '精品店', '用品店', '日用品店', '产品店', '便利店', '加盟店', '旗舰店', '总店', '销售店', "代理店", "专属代理店", '平价综合商店',
                 ...
                 '便民店', '器材店', '建材店', '眼镜店', '装饰店', '手机店', '材料店', '科技店', '电器店', '加工店', '食品超市', '水果超市',
                 '维修店', '配件店', '文具店', '玩具店', '服务店', '广告店', '饰品店', '花苑', '批零部', '门窗店', '内衣店', '女装店', '童装店', '服饰店',
    ...
    '商号': {'.': types.tags_NO, '-': {'号角', '号手', '号声'},
           '+': {'南货号', '山茶号', '自行车号', '仓买', '商行', '盐仓', '琴行', '茶行', '酒行', '金行', "漆店", "鞋行", "烟店", "布行", '柜台', '专柜', '药柜', '码头店', '书城', '卖场', '商都', '商城店', }},
    "制品店": {'.': types.tags_NM, '-': {},
            '+': {'名酒汇', '美食汇', '大全', '大全店', '总汇店', "玻璃店", "陶瓷店", "数码店", "家私店", '劳保店', '办公店',
                  ...
                  '商场店', '市场店', '广场店', '中心广场店', '机场店', '飞机场店', '体育场店', '商贸城店', '锁城店', '大学店', '居然之家店', '公寓店', '书屋店', '供应店', '京东店'}},
    '代办柜': {'.': types.tags_NM, '-': {'柜子'},
    ...
    '火车站': {'.': types.tags_NM, '-': {},
            '+': {'汽车站', '中心站', '汽车客运站', '铁路专用线', '站台', '枢纽台', '发运站台',
                  "中转站", "运输站", "储运站", "货运站", "转运站", "收费站", "配送站", "收储站", '铁路港'}},
    '总站': {'.': types.tags_NO, '-': {''},
           '+': {'车站', '东站', '北站', '南站', '西站', "货站", '码头', '货仓', }},
    '货运部': {'.': types.tags_NM, '-': {},
