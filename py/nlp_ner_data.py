...
nt_tails = {
    ...
    '分店': {'.': types.tags_NB, '-': {},
           '+': {'分号', '分厂', '分矿', '分校', '分团', '分公司', '子公司', '分工厂', '分司', '分工司', '支公司', '分部', '经营分部', '分局', '分院', '分所', '直属分局', '支局', '分园', '子厂', '分场',
                 ...
                 '支中心', '分中心', '有限分公司', '酒店分公司', '大酒店分公司', '分院区', '分药房', '分苗圃', '分车库'}},
    ...
    '管理大队': {'.': types.tags_NM, '-': {},
             '+': {'地质大队', '调查大队', '边防支队', '消防支队', '改造支队', '管教支队', '劳改支队', '监管支队', '管理支队', '综合大队', '钻井大队', '物探大队', '保障支队', '海防支队', '巡逻支队',
                   ...
                   '交警大队', '城监大队', '消防大队', '执法中队', '水政监察大队', '渔政监督大队'}},
    '海关': {'.': types.tags_NM, '-': {'关于'},
           '+': {"商务部", }},
    '办公厅': {'.': types.tags_NM, '-': {},
    ...
    '支行': {'.': types.tags_NB, '-': {},
           '+': {'中心支行', '大学支行', '广场支行', '开发区支行', '小微支行', '火车站支行', }},
    # 供销合作
    '服务社': {'.': types.tags_NM, '-': {},
    ...
    '商店': {'.': types.tags_NM, '-': {'店内', '门市内', '市面'},
           '+': {'专门店', '专营店', '专业店', '精品店', '用品店', '日用品店', '便利店', '加盟店', '旗舰店', '总店', "专属代理店", '平价综合商店', '直营店', '中心超市', '平价菜铺', '平价商店',
                 '用品门市', '销售门市', '器材门市', '建材门市', '通讯门市', '收购门市', '生资门市', '超市', '平价超市', '便利超市', '用品超市', "精品屋", "专店", "专卖店",
                 '小超市', '便民超市', '生活超市', '百货超市', '烟酒超市', '连锁超市', "收购店", "代销店", "分销店", "直销店", "门业店",
                 '便民店', '水果超市', '总代理', '食品超市', '形象店', '普通店', '中心门店', '经营坊', }},
    ...
    '五金店': {'.': types.tags_NO, '-': {},
            '+': {'艺品店', '生活馆', "家电店", '饰品店', "钢材店", "水泥店", "卫浴店", "宠物店", "理疗店", "瓷砖店", "护肤店", '傢俬店', '家俬店', '仓储店', "美妆店", "孕婴店", "装修店", "保健店", "工具店",
                  ...
                  '电器店', '加工店', "玉器店", "花圈店", '通讯店', '农资店', '电脑店', '产品店', '经营店', '妆品店', '批发店', '卖店', '化妆品店', '特产店', '小卖店', "双代店", '零售店', '经销店', '销售店',
                  "代理店", "电动车店", "护肤品店", "不锈钢店", '综合门市', }},

    '坊店': {'.': types.tags_NO, '-': {},
           '+': {'金店', '花店', '包店', '锁店', "花坊", '货店', "布店", '之家', '花舍', '衣舍', '鞋店', '门市', '名店', '新店', '支店', '阁店', '书店', '鞋荘', "商社", '中心店',
                 "道馆", '书屋', '沙龙', '门店', '花苑', "表店", '专档', "煤店", "网店", }},
    # 地标店铺
    '万达店': {'.': types.tags_NO, '-': {},
    ...
    '中华人民共和国': {'.': types.tags_NZ, '-': {},
                '+': {'中国共产党', '共产党', '中国共青团', '共青团', '中国人民解放军', '中国人民武装警察部队', '中国人民武警部队', '中国人民政治协商会议', '中国人民政协',
                      }},
...
class tree_paths_t:
    ...
    score_maps = {'X': 0, 'U': 3, 'A': 5, 'N': 6, 'H': 7, 'S': 7, 'Z': 8, 'O': 8, 'B': 7, 'M': 8, 'L': 9, }
    ...
    def score(seg, nseg, txt):
        ...
        if nseg[0] > seg[1]:  # 前后段分离
            ...
            if seg[1] - nseg[0] >= 2:  # 特定重叠多字的情况
                ...
                if nseg[1] - seg[0] == 3 and seg[2] & {types.NA, types.NN, types.NU} and nseg[2] & {types.NA, types.NN, types.NU}:
                    ds = 0
                elif nseg[1] - seg[0] == 3 and seg[2] & {types.NU} and nseg[2] & {types.NB, types.NO}:
                    ds = 0
                elif seg[2] & {types.NA, types.NN, types.NU} and nseg[2] & {types.NA, types.NN, types.NU}:
                    ds = 1
                else:
                    ds = (seg[1] - nseg[0]) * 2
    ...
    def _loop(self, txt, segs, pnode, badlimit=4):
        ...
        for nidx in nexts:
            nseg = segs[nidx]
            score = tree_paths_t.score(pseg, nseg, txt)  # 当前路径节点分
