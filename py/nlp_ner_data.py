...
class types(Enum):
    ...
    def type(val):
        ...
        def by_num(num):
            """根据数字查找对应的主要组份类别"""
            assert num >= types.SP.value
            if types.NS.value <= num <= types.NS5.value:
                return types.NS
    ...
    @staticmethod
    def equ(v, t, strict=False):
        """判断给定的值类型v是否等同于目标类型t(多类型时按最大值判断)."""
        return types.cmp(v, t, strict) == 0

    @staticmethod
    def cmp(v, t, strict=False):
        ...
        def conv(val):
            ...
            if not strict:
                val = types.type(val)  # 非严肃数值比较,则进行基准归类处理

            if isinstance(val, int):
                if val < types.SP.value:
                    return None  # 要求不能单独使用行业类别整数
                return types.type(val)
            elif isinstance(val, types):
                if val.value < types.SP.value:
    ...
    def joint(v, t):
        ...
        def make_set(val):
            ...
            def rec(v):
                if isinstance(v, types) and v.value < types.SP.value:
                    return
                elif isinstance(v, int) and v < types.SP.value:
                    return
...
nt_tails = {
    ...
    '矿区': {'.': {types.NM, types.mining}, '-': {},
           '+': {'矿井', '煤矿', '矿山', '金矿', '铁矿', '气矿', '盐矿', '石矿', '砂矿', '硅矿', '钙矿', '钛矿', '钨矿', '铅矿', '钼矿', '铀矿', '铜矿', '铝矿', '锡矿', '硫铁矿',
                 '锌矿', '锰矿', '联营矿', '金属矿', '原料矿', '有色矿', '露天矿', '石灰石矿山', '重晶石矿山', '煤矿区', '磷矿'}, '-': {'采矿', '探矿', '洗矿', '矿长', }},
    ...
    '回收站': {'.': {types.NM, types.env}, '-': {},
            '+': {'回收', '回收拆解站', '回收处', '回收厂', '回收点', '回收行', '回收中心', '回收部', '回收利用部', '回收利用站', '回收店', '回收队', '废品回收',
                  '物质回收', '资源回收', '物资回收', '再生资源回收', '收购站', '废品收购站', '废品回收场', '废旧品回收场', '回收场'}},
    # 政府司法
    ...
    '委员会': {'.': {types.NM, types.gov}, '-': {},
            '+': {'工会委员会', '工作委员会', '管理委员会', '常委会', '人大常委会', '管委会', '组委会', '委会', '区委', '县委', '纪委', '团委',
    ...
    '饭店': {'.': {types.NM, types.catering}, '-': {},
           '+': {'快餐', '餐饮店', '食店', '冷饮店', '饼店', '酒楼', '大酒楼', '大饭店', '酒家', '大酒家', '食堂', '大食堂', '职工食堂', '餐厅', '烧烤广场', '啤酒广场', '美食广场', '餐饮广场'}},
    ...
    '家常菜': {'.': {types.NM, types.catering}, '-': {},
            '+': {'私房菜', '小饭桌', '小餐桌', '小吃', '汤馆', '粉馆', '肉馆', '酒馆', '面馆', '鱼馆', '农家菜馆', '洒家', '渔家', '酒玩家', '饼家', '小吃坊', '小吃亭', '小吃城', '快餐城', '海鲜城', '小吃屋'}},
    ...
    '民宿': {'.': {types.NM, types.tourism}, '-': {},
           '+': {'公寓', '客莊', '山莊', '庄园', '农庄', '山庄', '大院', '居士林', '蒙古大营', '傣家园', '农家园', '残疾人之家', '农家', '老年公寓', '商务公寓',
    ...
    '商场': {'.': {types.NM, types.retail}, '-': {},
           '+': {'服务都', '银都', '书城', '商城', '卖场', '商都', '购物广场', '小世界', '大世界', '商厦', '商业大厦', '商埸', "展场", "家具广场", "商业广场", '通信广场', '家俬广场',
    ...
    '码头': {'.': {types.NM, types.traffic}, '-': {},
           '+': {'渡口', '港口', '航空港', '港湾', '机场', '国际机场', '客运主枢纽', '航电枢纽', '重件码头', '服务码头', '装卸码头', '渔港监督', '粮港', '联运港', '货运港', '储运港'}},
    ...
    '管理站': {'.': {types.NM, types.gov}, '-': {},
            '+': {'管理总站', '检查站', '边防检查站', '植检站', '植保植检站', '检测站', '监测站', '环境监测站', '水利管理站', '救助管理站', '监督管理站', '工程管理站', '建设管理站',
                  '公路管理站', '检疫站', '监督站', '质量监督站', '服务股', '管理股', '工程股', '防火站', "防疫站", "安监站", "统计站", "保护站", "防治站", "监理站", "检验站", }},
    ...
    '服务队': {'.': {types.NM, types.service}, '-': {},
            '+': {'维修队', '劳务队', '装卸队', '施工队', '工程队', '土石方工程队', '建筑工程队', '建筑队', '测绘队', '修缮队', '安装队', '设计队', '养护队', '城调队',
                  '救护队', "专业队", "工作队", "绿化队", "监察队", "搬运队", "测量队", '生产队', '牧队', '牧业队', '林业队', '中队', '井队', '大队', '总队', '支队', '车队'}},
    ...
    "鱼场": {'.': {types.NM, types.fishery}, '-': {},
           '+': {'渔塘', '鱼池', '渔港', '养殖基池', '基池', '养鱼池', '鱼池', '养鱼塘', "渔场", '鱼塘', '鱼苗场', '金鱼池', '鱼原种场', '水产渔场', '鱼苗鱼种场', '鱼种场', '水产试验场'}},
    '种植场': {'.': {types.NM, types.farming}, '-': {},
            '+': {'花圃', '苗圃', '茶园', '菜园', '果园', '园艺', '大棚', '种植园', '农场', '木场', '林场', '果场', '苗场', '茶场', '菜场', '花木场', '竹木场', '储林场', '农作物种植场', "实验农场",
                  '苗木基地', '种植基地', '植物园', '花园', '苗圃场', '花木园', '国有林场', '园艺场', '绿化园', '农业园', '花卉园', '生态园', '大棚', '温室大棚', '核桃园', '樱桃园', '农作物原种场',
                  '蔬菜温室大棚', '养殖地', '橡胶地', '种植地', '总埸', '林埸', '种植示范小区', '营林区', "发展场", "农林发展场", "农业发展场", "蔬菜发展场", '示范农场', '经营林场', '实验林场', '林业场',
    ...
    # 开发区/地区/园区
    '地区': {'.': {types.NS3}, '-': {},
           '+': {'新建区', '新区', '托管区', '保护区', '经开区', '管区', '保护开发区', '保税区', '边境经济合作区', '产业集聚区',
                 ...
                 '生态经济开发区', '示范区', '特别合作区', '特区', '特色农业经济开发区', '外商投资工业区', '高技术产业开发区', '外向型农业开发区', '综改区',
    ...
    '园区': {'.': {types.NS4, types.NM, types.service, types.industry}, '-': {},
           '+': {'采区', '产业园区', '城市工业园区', '发展园区', '服务区', '高教园区', '高科技园区', '高科园区', '高新技术产业园区', '高新技术示范园区', '工业园区', '工业园小区', '广场', '农高区', '科创区', '高科园',
                 '国家农业园区', '化工园区', '教区', '教育园区', '经济工业园区', '酒业园区', '科工贸园区', '科技园区', '空港物流园区', '临港化工园区', '临港物流园区', '牧业园区', '农业高新技术园区', '警备区',
                 '农业科技园区', '农业园区', '汽车园区', '商务产业园区', '深加工园区', '生态工业园区', '示范小区', '停车区', '微电子小区', '温泉休闲商务产业园区', '物流园区', '物业区', '现代农业园区', '空港区',
                 ...
                 '特别行政管理区', '农垦管理区', '华侨管理区', '经济管理区', '片区', '其他片区', '引灌区', '灌区', '垦区', '围垦区', '库区', '水库灌区', '资源区', '管护区', '林区', '航空港区', '保税港区', '核心港区'}},
    ...
    '社区': {'.': {types.NS5, types.NM, types.gov}, '-': {},
           '+': {'街道', '集体资产社区', '组团虚拟社区', '直属社区', '直属虚拟社区', '虚拟社区', '虚拟生活区', '生活区', '类似社区', '联合小区', '产业社区', '工业社区', '社区工作站', '类似乡级单位', '特殊社区',
                 '直辖村级区划', '住宅区', '新型农村社区', '中心社区', '中心街道', '虚拟劳管所', '类似村', '公司村', '区块社区', '社区村', '城市社区', '文化社区', '居民区', '小区'}},
    ...
    '局': {'.': {types.NO, types.gov}, '-': {'局长', '局部', '局座', '局域', '本局', '我局'}, '+': {}},
    '厅': {'.': {types.NO}, '-': {'厅长'}, '+': {}},
    '行': {'.': {types.NO}, '-': {'行长'}, '+': {}},
    '团': {'.': {types.NO}, '-': {'团长', }, '+': {}},
    '房': {'.': {types.NO}, '-': {'房子', '房间', '房内'}, '+': {}},
