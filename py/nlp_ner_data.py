...
class types(Enum):
    ...
    NS1 = 151  # 省/自治区/直辖市
    NS2 = 152  # 地级市/州/盟
    NS3 = 153  # 区/县/旗 (地区/开发区/景区)
    NS4 = 154  # 乡镇/街道/苏木 (园区/工业园/度假区)
    NS5 = 155  # 村屯/社区/嘎查/农场/林场/牧场/渔场 (公园/游乐园)
    ...
    def type(val):
        ...
        if tn in {'set', 'list', 'tuple'}:
            val = max(val)
            tn = type(val).__name__

        if tn == 'types':
            return _maps[val.value]
        else:
            return _maps[val]
    ...
    def trade(tags):
        ...
        rst = []
        for t in tags:
            if t.value < types.SP.value and t not in rst:
                rst.append(t)
        return rst
    ...
    def joint(v, t):
        ...
        def make_set(val):
            ...
            def rec(v):
                tn = type(v).__name__
                if tn == 'types' and v.value < types.SP.value:
                    return
                elif tn == 'int' and v < types.SP.value:
                    return
...
nt_tails = {
    ...
    '分店': {'.': {types.NB}, '-': {},
           '+': {'分号', '分厂', '分矿', '分校', '分公司', '子公司', '分司', '分工司', '支公司', '分部', '分局', '分院', '分所', '直属分局', '支局', '分园', '子厂', '分场', '分社',
                 '分站', "分会", "子站", "分支机构", "分队", "分处", '分区', '分台', '(分)局', '名胜区分公司', '机场分公司', '林场分公司', '牧场分公司', '火车站分公司', '火车站分社',
                 '中心分社', '中心支局', '中心支公司'}},
    '工厂': {'.': {types.NM, types.industry}, '-': {},
           '+': {'加工厂', '化工厂', '工区', '件厂', '剂厂', '布厂', '床厂', '总厂', '机厂', '材厂', '板厂', '水厂', '油厂', '泵厂', '工廠', '净水厂', '纯水厂', '纯净水厂',
    ...
    '公司': {'.': {types.NM, types.com}, '-': {},
           '+': {'总公司', '有限公司', '有限责任公司', '责任公司', '股份公司', '集团公司', '集团有限公司', '集团有限责任公司', '集团总公司', '株式会社', '顾问有限公司', '连锁有限公司',
                 ...
                 '建设发展有限公司', '设计有限公司', '建设有限公司', '制造有限公司', 'LTD', '劳司', '工司', '有限责任工司', '农工商联合公司', '农工商公司', '农工商总公司'}},
    ...
    '普通合伙': {'.': {types.NM, types.com}, '-': {'合伙人'},
             '+': {'管理合伙', '咨询合伙', '农业合伙', '有限合伙', '房地产合伙', '投资合伙'}},
    ...
    '办公厅': {'.': {types.NM, types.gov}, '-': {},
            '+': {'合作厅', '财政厅', '自然资源厅', '自然资源和规划厅', '经济和信息化厅', '管理厅', '科技厅', '科学技术厅', '监督管理厅',
    ...
    '办公室': {'.': {types.NM, types.gov}, '-': {},
            '+': {'管理办', '项目办', '研究室', '教学研究室', '政策研究室', '工作办', '法制办', '开发办', '综合开发办', '政府办', '接待办', '建设管理办', '领导小组办', '建设办', }},
    # 学校
    ...
    '学校': {'.': {types.NM, types.edu}, '-': {'学业'},
           '+': {'小学', '小学校', '中学', '中学校', '高中', '中心校', '总校', '中心学校', '武校', '盲校', '网校', '聋校', '联校', '公学', '职业中学', '附属中学', '中心小学校', '中心中学',
                 ...
                 '校区', '二校区', '东校区', '南校区', '西校区', '北校区', '中心校区', '学区', }},
    ...
    '协会': {'.': {types.NM, types.org}, '-': {},
           '+': {'企业协会', '行业协会', '服务协会', '警察协会', '老年协会', '技术协会', '科学技术协会', '职工技术协会', '管理协会', '教育学会', '图书馆学会', '总会', '交流协会',
                 '志愿者协会', '用水者协会', '体育协会', '计划生育协会', '校友会', '商会', '学会', '研究会', '联谊会', '促进会', '体协', '技协', '消协', '职工技协', "持股会",
                 "职工持股会", '(协)会', }, '-': {'协助', '协调'}},
    ...
    '委员会': {'.': {types.NM, types.gov}, '-': {},
            '+': {'工会委员会', '工作委员会', '管理委员会', '常委会', '人大常委会', '管委会', '组委会', '委会', '区委', '县委', '纪委', '团委', '市委', '省委', '地委',
                  ...
                  '纪工委', '纪检委', '计生委', '生态管理委员会', '建设管理委员会', '筹委会', '监事会', '中心委员会', '中心校委员会', '中心局委员会',
                  }},
    '居委会': {'.': {types.NM, types.gov}, '-': {},
            '+': {'居民委', '居民委员会', '居民居委会', '民居委员会', '居民委会', '治理委员会', '居民事务中心', '居委', '街民委员会', '居民管理委员会', '家委会', '街道委员会', '街道办事处', '办事处', '街道办',
                  '类似居委会', '商务公寓工作站', '民主议事会', '居民议事会', '社区工作站', "团场"}},
    ...
    '医院': {'.': {types.NM, types.medical}, '-': {},
           '+': {'中医院', '妇产医院', '中医医院', '康复医院', '附属医院', '职工医院', '中心医院', '总医院', '人民医院', '动物医院', '宠物医院', '精神病医院', "血站",
                 ...
                 '中心卫生院', '中心血站', '脑科医院', '眼科医院', '连锁门诊部', '中医堂'}},
    ...
    '饭店': {'.': {types.NM, types.catering}, '-': {},
           '+': {'快餐', '餐饮店', '食店', '冷饮店', '饼店', '饭馆', '酒楼', '大酒楼', '大饭店', '酒家', '大酒家', '食堂', '大食堂', '职工食堂', '餐厅', '烧烤广场', '啤酒广场', '美食广场', '餐饮广场'}},
    '茶馆': {'.': {types.NM, types.catering}, '-': {},
           '+': {'茶社', '茶庐', '茶楼', '茶舍', '茶座', '茶都', '茶莊', '茶艺居', '茶艺店', '茶艺苑', '茶饮店', '茶饮坊', '茶叶店', '茶居', '茶荘', '茶庄', '茗茶轩', '茶轩', '茶饮室', '茶艺坊', '茶艺室',
    ...
    '商店': {'.': {types.NM, types.retail}, '-': {'店内', '市内', '市面'},
           '+': {'门店', '专门店', '专营店', '专业店', '小店', '精品店', '用品店', '日用品店', '产品店', '便利店', '加盟店', '旗舰店', '总店', '销售店', "代理店", "专属代理店",
                 ...
                 '用品门市', '销售门市', '器材门市', '建材门市', '通讯门市', '收购门市', '生资门市', '超市', '平价超市', '便利超市', '用品超市', "精品屋", "专店",
                 '小超市', '便民超市', '生活超市', '百货超市', '烟酒超市', '连锁超市', '书屋', '通讯店', '农资店', '电脑店', "收购店", "代销店", "分销店", "直销店", "门业店",
                 '五金店', '金店', '花店', '便民店', '器材店', '建材店', '眼镜店', '装饰店', '手机店', '材料店', '科技店', '电器店', '加工店', '艺品店', "花坊", '火车站超市',
                 '维修店', '配件店', '文具店', '玩具店', '服务店', '广告店', '饰品店', '超市店', '中心店', '包店', '锁店', '鞋店', '花苑', '中心文具店',
                 ...
                 "家电店", "理疗店", "瓷砖店", "护肤店", '傢俬店', '家俬店', '仓储店', '中心门店', '专档', '经营坊', '形象店', }},
    ...
    '商行': {'.': {types.NM, types.retail}, '-': {},
           '+': {'琴行', '茶行', '车行', '酒行', '金行', '总汇', '傢俬', '家俬', '名酒汇', '美食汇', '大全', '大全店', '总汇店', "烟店", "玻璃店", "陶瓷店", "数码店", "家私店", '劳保店', '办公店',
    ...
    '码头': {'.': {types.NM, types.traffic}, '-': {},
           '+': {'航空港', '机场', '国际机场', '客运主枢纽', '航电枢纽', '重件码头', '服务码头', '装卸码头', '渔港监督', '粮港', '联运港', '货运港', '储运港'}},
    ...
    '货运部': {'.': {types.NM, types.traffic}, '-': {},
            '+': {'物资部', '运输部', '配送部', '发行部', '汽车队', '运输队', '运输户', '机修班', '汽车班', '运输班', "托运部", "物流部", }},
    '仓库': {'.': {types.NM, types.logistics}, '-': {'库内', '库存', '库中'},
           '+': {'储备库', '粮食储备库', '直属库', '粮库', '仓储部', '仓储场', '仓储站', '粮站', '货仓', '资料仓', '配送仓', '供应仓', '保税仓', '保管仓', '前置仓', '副食仓', '化肥仓', '工艺仓', '布仓',
                 ...
                 '储运库', '直管库', '石油气库', '糖库', '装备库', '配送库', '仓储栈', '储运栈', '转运栈', '煤栈', '煤炭仓储栈', '货栈', '贸易栈', '贸易货栈', '暖车库', '车库', '保管场', '中心仓库', '库区', }},
    ...
    '服务站': {'.': {types.NM, types.retail}, '-': {},
            '+': {'工作站', '维修站', '汽车维修站', '劳动服务站', '文化站', "批发站", "建设站", "采购站", "物资站", "试验站", "指导站", "搅拌站", "经营站", "经销站", "军供站", "保健站",
                  "妇幼保健站", "救助站", "供热站", "修理站", "护理站", "仪器站", "接待站", "水土保持站", "交流站", }},
    ...
    "鱼场": {'.': {types.NM, types.fishery}, '-': {},
           '+': {'渔塘', '鱼池', '渔港', '养殖基池', '基池', '养鱼池', '养鱼塘', "渔场", '鱼塘', '鱼苗场', '鱼原种场', '水产渔场', '鱼苗鱼种场', '鱼种场', '水产试验场'}},
    '种植场': {'.': {types.NM, types.farming}, '-': {},
            '+': {'花圃', '苗圃', '茶园', '果园', '种植园', '农场', '木场', '林场', '果场', '苗场', '茶场', '菜场', '花木场', '竹木场', '储林场', '农作物种植场', "实验农场", '中心苗圃',
                  ...
                  '养殖地', '橡胶地', '种植地', '总埸', '林埸', '种植示范小区', '营林区', "发展场", "农林发展场", "农业发展场", "蔬菜发展场", '示范农场', '经营林场', '实验林场', '林业场', '兰花园',
                  ...
                  '桔场', '茶叶示范场', '园艺试验场', '林场区', '林木场', '桂花园', "草籽场", '农业场', '花卉繁殖场', '果树繁殖场', '良桑繁殖场', '原种繁殖场', '种籽繁殖场', '苗种繁殖场', '园艺繁殖场',
    ...
    '养殖场': {'.': {types.NM, types.breeding}, '-': {},
            '+': {'猪场', '羊场', '马场', '鸡场', '养殖基地', '水产养殖场', '猪养殖场', '生猪养殖场', '垦殖场', '牧场', '原种场', '养殖埸', '养殖总埸', '养殖总场', '农牧场', '种羊场', '水产场', '山羊场',
                  ...
                  '养蚕场', '养殖试验场', '增养殖试验场', '种养殖试验场', '畜牧试验场', '鸽场', '养龟场', '种鳖场', '鳗场', '畜禽试验场', '种猪试验场', '鱼种试验场', '种马场', '种蜂场', '种蓄场'}},
    ...
    '国营': {'.': {types.NF, types.com}, '-': {},
           '+': {'中交', '中建', '国网', '国电', '中烟', '中电', '中电建', '中铁建', '国家', '华能', '华电', '华润', '中铁', '国有', '兵团', '中粮', '生产建设兵团'}},

    # 地区/方位
    '中央': {'.': {types.NS}, '-': {},
           '+': {'东', '南', '西', '北', '华北', '东北', '华东', '华中', '华南', '西南', '西北', '东部', '中部', '西部', '中南', '北方', '南方', '东南', '中区', '东区', '南区', '西区', '北区', }},

    # 开发区/地区/园区
    '地区': {'.': {types.NS3}, '-': {},
           '+': {'新建区', '新区', '托管区', '保护区', '经开区', '管区', '保护开发区', '保税区', '边境经济合作区', '产业集聚区', '经发区',
                 ...
                 '经济技术开发区', '经济开发区', '经济开发试验区', '经济开发新区', '经济贸易开发区', '经济区', '经济特区', '经济综合实验区', '军区', '军分区', '开发分区', '开发区', '开放开发区', '科技经济开发区',
                 ...
                 '生态移民示范区', '苏木', '新型工业化化工区', '农村牧区', '牧区', '边境经济技术合作区', '经济技术合作区', '新兴产业示范区', '易地扶贫搬迁安置区', '健康产业国际合作示范区', '滨海高新区',
                 '综合物流产业聚集区', '自由贸易试验区', '高新技术产业区', '特殊乡镇', '直属乡镇', '工贸集中区', '民营经济区', '农业高新技术示范区', '新经济区', '协同发展区', '产业开发区', '统筹发展试验区',
                 '商贸物流枢纽区', '投资区', }},
    '景区': {'.': {types.NS3, types.cultural}, '-': {},
           '+': {'风景旅游区', '风景名胜区', '风景区', '风景游览区', '名胜风景区', '旅游度假区', '旅游渡假区', '旅游管理区', '旅游经济开发区', '旅游景区', '旅游开发区', '旅游区', '温泉风景区', '温泉疗养旅游开发区',
                 '生态休闲旅游度假区', '滨海旅游度假区', '滑雪旅游度假区', '生态休闲区', '文化旅游区', '生态旅游度假区', '省级旅游度假区', '生态旅游建设示范区', '生态旅游示范区', '自然保护区', '度假区', '边贸旅游区',
                 '滨海旅游渡假区', '省级生态旅游度假区', '渡假区', '国家级自然保护区', '省级自然保护区', '海湾旅游区', '生态保护区', '生态管理区', '生态旅游区', '生态示范区', '游览区', '历史文化街区', '温泉新区',
                 '商业文化旅游中心区', '生态文化旅游示范区', '文化景区', '生态文化旅游区', '风景名胜管理区', '生态区'}},
    '园区': {'.': {types.NS4, types.NM, types.service, types.industry}, '-': {},
           '+': {'采区', '产业园区', '城市工业园区', '发展园区', '服务区', '高教园区', '高科技园区', '高科园区', '高新技术产业园区', '高新技术示范园区', '工业园区', '工业园小区', '农高区', '科创区',
                 '国家农业园区', '化工园区', '教育园区', '经济工业园区', '酒业园区', '科工贸园区', '科技园区', '空港物流园区', '临港化工园区', '临港物流园区', '牧业园区', '农业高新技术园区', '警备区', '新片区',
                 '农业科技园区', '农业园区', '汽车园区', '商务产业园区', '深加工园区', '生态工业园区', '示范小区', '停车区', '微电子小区', '温泉休闲商务产业园区', '物流园区', '物业区', '现代农业园区', '空港区', '功能区',
                 '新产业园区', '新技术产业园区', '新型工业园区', '循环经济产业园区', '院区', '中心商务区', '科技商务区', '专利技术园区', '工业园管理区',
                 '制造工业园区', '承接产业转移集中示范园区', '集中示范园区', '现代产业园区', '生态农业园区', '现代服务业园区', '农副产品加工交易园区', '交易园区', '农业园区', '商务商贸街区', '现代农牧业园区', '农牧业园区',
                 '生物科技产业园区', '农机产业园区', '装备制造产业园区', '现代农业产业园区', '农业产业园区', '家纺产业园区', '现代农业示范园区', '农业示范园区', '能源化工园区', '厂区', '国家农业科技园区', '港区',
                 '轻工业园区', '新材料园区', '高新产业园区', '职业教育园区', '循环经济园区', '循环园区', '安置区', '临空综合经济园区', '新兴产业工业园区', '工业示范区', '生态经济园区', '出口创汇示范园区', '储粮区',
                 '乐器工业园社区', '港务区', '国际港务区', '实验区', '作业区', '加工区', '国际物流加工区', '工业加工区', '出口加工区', '外向型工业加工区', '管理区', '特别管理区',
                 '特别行政管理区', '农垦管理区', '华侨管理区', '经济管理区', '垦区', '围垦区', '洪泛区', '资源区', '管护区', '林区', '航空港区', '保税港区', '核心港区'}},

    # '病区': {'.': {types.NS4, types.NM, types.medical}, '-': {},
    #        '+': {'疗区','监区','矿区}},
    ...
    '社区': {'.': {types.NS5, types.gov}, '-': {},
           '+': {'街道', '集体资产社区', '组团虚拟社区', '直属社区', '直属虚拟社区', '虚拟社区', '虚拟生活区', '生活区', '类似社区', '联合小区', '产业社区', '工业社区', '类似乡级单位', '特殊社区',
                 '直辖村级区划', '住宅区', '新型农村社区', '中心社区', '中心街道', '虚拟劳管所', '类似村', '公司村', '社区村', '农场村', '实验村', '渔场村', '林场村', '区块社区', '城市社区', '文化社区', '居民区', '小区',
                 '火车站社区', '花园'}},
    '公园': {'.': {types.NS5, types.NM, types.cultural}, '-': {},
           '+': {'乐园', '游乐园', '游乐场', '游乐城', '音乐城', '森林公园', '动物园', '风景园', '娱乐湾', '赛马场', '骝马场', '走马场', '溜冰城', '旱冰城', '文化城', '动漫城', '广场', '中心广场', }},
