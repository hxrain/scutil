...
class nt_parser_t:
    ...
    num_rules = [(f'([第笫苐新老大小东西南北钢省市区县村镇乡附]?[\\.{nnp.num_re}]{{1,7}}[#号號户轮块度角毛分秒吨届座级期船至元克机天年℃°]?)', types.tags_NU, __nu_rec.__func__),
                 (f'([第笫苐新老大小东西南北]?[{nnp.num_re}]{{1,7}}[#号號级大支]*)(公里|经路|纬路|经街|纬街|马路|路段|矿区|社区|组村|队组|组组|职高|职中|层楼|[职委米条轮船道路弄街口里亩线层楼栋幢段桥井闸渠河沟江坝村区片门台房田居营连排])',
                  types.tags_NS, __nu_rec.__func__),
                 (f'([第笫苐新老大小东西南北]*[{nnp.num_re}]{{1,7}}[号號]?)(营部|院区|柜组|部队|煤矿|船队|茶楼|包房|站点|坊厅)', types.tags_NO, __nu_rec.__func__),
    ...
    def rec_nums(segs, txt):
        ...
        def skip_next(pos, uidx, usegs):
            ...
            w_nexts = {'工区', '分号', '部队', '公里', '马路', '矿区', '社区', '号仓', '分钟', '小时', '职高', '大道', '院区', '支部', '号店', '船队', '号楼', '分场',
                       '路段', '分店', '分仓', '站点', '仓库', '库房', '坊厅', '号库', '校区', '号闸', '号厅', '号亭', '号铺', '号坊'}
    ...
    def _tidy_segs(matcher, segs, merge_seg, line_txt):
        ...
        def can_combi_NS(pseg, seg, txt, nseg=None):
            ...
            if ct in {'区', '县', '乡', '镇', '村', '屯', '居', '办', '组', '港', '港区', '湾区', '地区', '苏木', '嘎查', '小区', '街道', '社区', '行政村', '自然村'}:
                ...
                if ct in std_ns_tails and nseg and nseg[2] & {types.NO, types.NM, types.NB, types.NO}:
                    return False  # 当前是区划特征字,后面是实体尾缀,不合并
            ...
            if pseg[1] > seg[0]:
                ...
                if pseg[1] - seg[0] == 1:  # 前后单字交叉
                    ...
                    if seg[0] - pseg[0] == 1:
                        ...
                        if line_txt[pseg[0]] in area1_chars and pseg[2] & {types.NA, types.NN, types.NS} and types.NS in seg[2]:
                ...
                if pseg[1] - pseg[0] == 1 and line_txt[pseg[0]] in area1_chars and pseg[2] & {types.NA, types.NN, } and types.NS in seg[2]:
                ...
                if pseg[1] - pseg[0] == 1:
                    ...
                    if pchar in areas_chars and seg[2] & {types.NB, types.NO, types.NM}:
                        rst[-1] = (pseg[0], seg[1], seg[2])
                        return True  # 特定前置单字连接特征尾缀,合并

                    if seg[1] - seg[0] == 2 and seg[2] & {types.NO, types.NM} and pchar in {'新', '老', '东', '南', '西', '北'}:
                        rst[-1] = (pseg[0], seg[1], seg[2])
        ...
        for idx in range(1, segs_len):
            pseg = rst[-1]
            seg = segs[idx]
            nseg = segs[idx + 1] if idx + 1 < segs_len else None
    ...
    def verify(self, name, segs=None, merge_seg=False, rec_NL=False, mres=None, pres=None, nres=None, strict=False, fp_dbg=False, chk_rst=False):
